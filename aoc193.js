...
const splitter = x => x.match(/([UDLR])(\d+)/);

const instructionsToWireList = instructions => {
    const wire = {};
    let x = 0;
    let y = 0;
    let index = 1;
...
const intersections = input => {
    ...
    const wireOneInstructions = wires[0].split(",").map(splitter);
    const wireTwoInstructions = wires[1].split(",").map(splitter);

    const wireOneList = instructionsToWireList(wireOneInstructions);
    const wireTwoList = instructionsToWireList(wireTwoInstructions);
    const duplicates = [];

    for (let x in wireOneList) {
        const ys = wireOneList[x];
        for (let y in ys) {
            if (wireTwoList[x] && wireTwoList[x][y])
            duplicates.push(`x${x}y${y}`);
        }
    }

    return {intersections: duplicates, wireOneList, wireTwoList};
};

const part1 = input => {
    const duplicates = intersections(input).intersections;
...
const part2 = input => {
    const result = intersections(input);
    const isx = result.intersections;
    const w1L = result.wireOneList;
    const w2L = result.wireTwoList;
    ...
    for (let dup of isx) {
        ...
        min = Math.min(min, w1L[x][y] + w2L[x][y]);
    }
